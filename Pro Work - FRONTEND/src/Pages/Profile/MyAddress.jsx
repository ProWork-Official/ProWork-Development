...
/* -------------------------------------------------------------------------- */
/* ----------------------------- UpdateLocationModal ------------------------ */
/* This is a direct, slightly adapted copy of your UpdateLocation component,  */
/* inlined so MyAddress no longer depends on the separate component file.     */
/* - It uses MyContext the same way and writes saved_addresses to localStorage */
/* - onClose() callback is used to notify parent (MyAddress) when modal closes */
/* -------------------------------------------------------------------------- */

function UpdateLocationModal({ onClose = null, autoOpenAdd = false }) {
  const { showMap, setShowMap, addss, setAdss, pickerStep, setPickerStep, statusMsg, setStatusMsg } =
  ...
  // Auto-start add-new when prop provided (runs once after mount)
    useEffect(() => {
      try {
        const v = localStorage.getItem("auto_open_add");
        if (v === "1") {
          localStorage.removeItem("auto_open_add");
          // startAddNewAddress should be the function inside the shared UpdateLocation
          // which performs geolocation + opens the map flow.
          if (typeof startAddNewAddress === "function") startAddNewAddress();
          else {
            // fallback: make the shared modal show map add-flow explicitly
            setPickerStep("map");
            // (optional) set any other local flags used by UpdateLocation
          }
        }
      } catch (e) {}
    }, []);
  ...
  // Autocomplete predictions (run when search screen or when map + add-new)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        if (listenerClick && window.google && window.google.maps && window.google.maps.event) window.google.maps.event.removeListener(listenerClick);
      } catch (_) {}
      if (mapInstanceRef.current) {
        try {
  ...
  const handlePredictionClick = (prediction) => {
    ...
    placesService.getDetails({ placeId: prediction.place_id, fields: ["geometry", "formatted_address", "name"] }, (place, status) => {
  ...
  // Quick flow: validate + save current location (non add-new)
  ...
  // Start Add New Address flow: center map on current location and keep isAddingNewAddress = true
  ...
  // Reverse geocode + validate coords
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        geocoder.geocode({ location: coords }, (results, geocodeStatus) => {
          ...
          const cityCandidates = [locality, sublocality, neighborhood, postalTown, formatted];
  ...
  const confirmLocation = async () => {
    if (!selected) {
      setStatusMsg("No location selected.");
      return;
    }
    const savedAddress = tempAdds || `${selected.lat.toFixed(6)}, ${selected.lng.toFixed(6)}`;
    const savedCoords = `${selected.lat},${selected.lng}`;

    // IMPORTANT: check both internal flag and parent-requested autoOpenAdd
    if (isAddingNewAddress || autoOpenAdd) {
      // open form to capture additional details and pre-fill completeAddress
      setAddressForm((prev) => ({ ...prev, completeAddress: savedAddress }));
      setShowAddressForm(true);
      return;
    }

    // default flow (not adding new): persist selection and close
    try {
  ...
  // helper to extract compact label from address string
  ...
  // Save address from the form to localStorage and notify parent
  const handleSaveAddress = (e) => {
    ...
    const updated = [...savedAddresses, newAddr].slice(0, 3);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="flex flex-col bg-white sm:rounded-lg w-full h-full sm:h-[80%] sm:max-w-xl sm:mx-4 md:mx-0 shadow-lg">
        <div className="flex justify-start p-2 py-4 shadow-md">
          <button className=" flex items-center text-sm" onClick={handleClose}>
