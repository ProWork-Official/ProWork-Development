...
function PersonalForm() {
  ...
  return (
    ...
    <div className="displayNone fixed inset-0 flex items-center justify-center z-50" id="PersonalDetailsForm">
  <div className="absolute inset-0 bg-black opacity-50"></div>
  <div className="relative flex h-full sm:h-auto w-full flex-col bg-white sm:w-[32rem] sm:rounded-2xl shadow-lg">
    <div className="flex items-center h-12 w-full pl-4">
      <img className="h-6 cursor-pointer rotate-180" src={next_icon} alt="back arrow" onClick={() => { togglePersonalForm(false) }} />
      <h3 className="text-md">
        <span className="text-gray-500 cursor-pointer" onClick={() => { togglePersonalForm(false) }}>Profile</span> / <span>Personal Details</span>
      </h3>
    </div>
    ...
    <form className="flex flex-wrap justify-center w-full py-8 md:py-2" onSubmit={handlePersonalSubmit}>
      ...
      <button
        className="h-12 w-[95%] border-2 border-[#f2da1d] rounded-xl shadow-lg shadow-[#33806b] bg-[#33806b] text-white"
        id="send-OTP-BTN"
        onClick={handlePersonalSubmit}
      >
        {loadingState ? (
          <div className="flex justify-center items-center">
            <img src={loadState} className="h-8" alt="" />
          </div>
        ) : (
          "Save Details"
        )}
