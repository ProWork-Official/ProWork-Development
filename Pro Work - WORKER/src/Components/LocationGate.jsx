...
export default function LocationGate({ children }) {
  ...
  const { status, setStatus, setAdss, setStatusMsg } = useContext(MyContext);
  ...
  useEffect(() => {
    ...
    // If a location exists in localStorage, use it
    const selected = localStorage.getItem("selected_coords");
    if (selected) {
      console.log("Coords taken from localStorage", selected);
    ...
    // otherwise use browser geolocation api to get your current location and use it
    const onPosition = (pos) => { 
      console.log("Taking coords from your current location", pos.coords.latitude, pos.coords.longitude);
      localStorage.setItem("selected_coords", `${pos.coords.latitude},${pos.coords.longitude}`);
    ...
    try {
      ...
      geocoder.geocode({ location: { lat, lng } }, (results, geocodeStatus) => {
        try {
          ...
          console.log('\n', "inBounds ==>", inBounds, '\n', "inUttarPradesh ==>", inUttarPradesh, '\n', "inPrayagraj ==>", inPrayagraj);
  ...
  // if status is checking, show checking page
  if (status === "checking") {
    return (
  ...
  // if status is outside, show change location page
  if (status === "outside") {
    return (
      <ChangeLocation />
    );
  ...
  // if status is allowed, show home page
  if (status === "allowed") {
    return (
      <>{children}</>
    )
  ...
  // if any location permission denied or any error
  return (
