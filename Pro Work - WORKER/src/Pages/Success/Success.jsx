...
import { toastSuccess } from '../../func.jsx';
...
function Success() {
  ...
  window.onload = function(){
    setTimeout(() => { toastSuccess("Payment successfull") }, 500); 
  }
  
  return (
    <div className='flex flex-col pt-20 w-screen mt-8'>
      <Helmet><title>Pro Work - Payment Successful</title></Helmet>
      <div className="w-full max-w-[1250px] mx-auto px-4 xl:px-0" >
        <h1 className='successh1 text-[2rem] sm:text-[2.5rem]'>Payment Successful</h1>
        <h3 className='my-4 font-semibold text-gray-700 mb-4'>
          <span className='text-black'>Payment ID = </span> 
          <span className='text-blue-600'>{query && query.get("payment_id")}</span>
        </h3>

        <div className="w-full max-w-[1250px] xl:px-0 pb-4">
          <h2>Thank you for your payment!</h2>
          <p>Your transaction has been successfully completed. <img src={check} alt="" className='h-8 py-1' /></p>
        </div>
